...
class datepickerview(BaseView):
    ...
    def choose_tomorrow_for_date_picker(self, strHour="11", strMinute="30", strMeridium="PM"):
        ...
        day_picker = self.waitForElement(locatorType=By.XPATH, locator=datepickerview._day_xpath)
        day_picker.send_keys(tomorrow)

        hour_picker = self.waitForElement(locatorType=By.XPATH, locator=datepickerview._hour_xpath)
        hour_picker.send_keys(strHour)

        minute_picker = self.waitForElement(locatorType=By.XPATH, locator=datepickerview._min_xpath)
        minute_picker.send_keys(strMinute)

        meridium_picker = self.waitForElement(locatorType=By.XPATH, locator=datepickerview._meridium_xpath)
